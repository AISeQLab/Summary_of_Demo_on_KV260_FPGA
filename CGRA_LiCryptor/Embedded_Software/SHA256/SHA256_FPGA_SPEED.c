...
int main(){
	...
	FILE *file1 = fopen("datatest_input_393216.txt", "r");  // Open file in read mode
    if (file1 == NULL) {
        perror("ERROR: Unable to open datatest_input_393216.txt file!");
        return 1;
    }
	...
	FILE *file3 = fopen("datatest_input_393216.txt", "r");  // Open file in read mode
    if (file3 == NULL) {
        perror("ERROR: Unable to open datatest_input_393216.txt file!");
        return 1;
    }
	...
	#endif
	Final();
	printf("CGRA execution time for %d inputs in miliseconds is %f\n", N_Data, (double)time_spent_cgra*1000 / BILLION); // Added this line for clarity
