...
#define P1(x) ((x) ^  ROTL((x),15) ^ ROTL((x),23)) 
	...
	for(j = 16; j < 68; j++ )
	{
		...
		Temp1 = W[j-16] ^ W[j-9]; // W[0] ^ W[7]
		Temp2 = ROTL(W[j-3],15); // ROTL(W[13],15)
		Temp3 = Temp1 ^ Temp2;  // 0x6164e387
		Temp4 = P1(Temp3);      // P1(x) ((x) ^  ROTL((x),15) ^ ROTL((x),23)) 
	...
	for(j =0; j < 16; j++)
	{
		...
		SS1 = ROTL((ROTL(A,12) + E + ROTL(T[j],j)), 7);     // 0x51368692
		SS2 = SS1 ^ ROTL(A,12);                             // 0x505071aa
		TT1 = FF0(A,B,C) + D + SS2 + W1[j]; //  0x699d4f85 + 0x505071aa = 0xb9edc12f
		TT2 = GG0(E,F,G) + H + SS1 + W[j];  //  0x6f315829 + 0x51368692 = 0xc067debb

		D = C;
		C = ROTL(B,9);  // 0x29657292
		B = A;
		A = TT1;
		H = G;
		G = ROTL(F,19); // 0xc550b189
		F = E;
		E = P0(TT2);
