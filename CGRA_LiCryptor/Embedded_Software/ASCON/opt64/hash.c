...
forceinline void ascon_absorb(state_t* s, const uint8_t* in, uint64_t inlen) {
  ...
  while (inlen >= ASCON_HASH_RATE) {
    // //printf("inlen = %d , ASCON_HASH_RATE = %d \n",inlen,ASCON_HASH_RATE);
	//printf(" s->x0 = %016lx\n",s->x0);
	//printf(" LOAD(in, 8)= %016lx\n",LOAD(in, 8));
    s->x0 = XOR(s->x0, LOAD(in, 8));
    ////printf("ASCON_HASH_ROUNDS = %d \n",ASCON_HASH_ROUNDS);
  ...
  if (inlen){
	  s->x0 = XOR(s->x0, LOAD(in, inlen));
	  //printf(" LOAD(in, inlen) = %016lx\n",LOAD(in, inlen));
  }
  // //printf("NEW 0 s->x0 = %016lx\n",s->x0);
  // //printf("PAD(inlen) = %016lx\n",PAD(inlen));
  s->x0 = XOR(s->x0, PAD(inlen));
  // //printf("NEW 1 s->x0 = %016lx\n",s->x0);
  P(s, 12);
...
forceinline void ascon_squeeze(state_t* s, uint8_t* out, uint64_t outlen) {
  ...
  while (outlen > ASCON_HASH_RATE) {
    // //printf("outlen = %d , ASCON_HASH_RATE = %d \n",outlen,ASCON_HASH_RATE);
    STORE(out, s->x0, 8);
    P(s, ASCON_HASH_ROUNDS);
    // //printf(" s->x0 = %016lx\n",s->x0);
    // //printf(" s->x1 = %016lx\n",s->x1);
...
int crypto_hash(unsigned char* out, const unsigned char* in,
  ...
  state_t s;
  ascon_hashinit(&s);
  // //printf(" s->x0-4 = %016lx%016lx%016lx%016lx%016lx\n",s.x0,s.x1,s.x2,s.x3,s.x4);
