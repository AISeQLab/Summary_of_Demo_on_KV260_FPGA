...
/*  ... 4GB(including REGS) */
#define LMM_BASE_PHYS	 0x00000000A4000000LL
/*  ... fixed */
...
#define STRIDE_BASE_PHYS	 (0x0000000000000040LL >> 2)
#define MAX_SESSION_BASE_PHYS	 (0x0000000000000060LL >> 2)
#define FINISH_BASE_IP   0x0000000000000000LL
...
struct cgra { /* host status of U2BCA */
...
//** For Transfer on ARM ZYNQ **//
  ...
  U32  *ctx_pe_mmap;     // user-virt Contiguous 4GB space including LMM space
  U32  *ctx_rc_mmap;     // user-virt Contiguous 4GB space including LMM space
...
int cgra_open()
{
  ...
  for (dir = 0; dir < num_dirs; ++dir) {
	...
	return -1;
	  ...
	  CGRA_info.ctx_pe_offset = CTX_PE_BASE_IP >> 2;
      CGRA_info.ctx_pe_mmap = (U32*)mmap(NULL, REG_MMAP_SIZE, PROT_READ|PROT_WRITE, MAP_SHARED, fd_reg, 0); /* 4GB */
      if (CGRA_info.ctx_pe_mmap == MAP_FAILED) {
		printf("ctx_pe_mmap failed. errno=%d\n", errno);
		return -1;
      }
	  
	  CGRA_info.ctx_rc_offset = CTX_RC_BASE_IP >> 2;
      CGRA_info.ctx_rc_mmap = (U32*)mmap(NULL, REG_MMAP_SIZE, PROT_READ|PROT_WRITE, MAP_SHARED, fd_reg, 0); /* 4GB */
      if (CGRA_info.ctx_rc_mmap == MAP_FAILED) {
		printf("ctx_rc_mmap failed. errno=%d\n", errno);
		return -1;
      }

	  CGRA_info.ctx_im_offset = CTX_IM_BASE_IP >> 2;
...
void dma_write_U32(U64 Offset, U64 size){
	...
	int status;
	  *(U64*)&(((struct dma_ctrl*)cgra.dma_ctrl)->ZDMA_CH_SRC_DSCR_WORD0) = DDR_BASE_PHYS + Offset;
      ((struct dma_ctrl*)cgra.dma_ctrl)->ZDMA_CH_SRC_DSCR_WORD2 = size*8*sizeof(U32);
	  *(U64*)&(((struct dma_ctrl*)cgra.dma_ctrl)->ZDMA_CH_DST_DSCR_WORD0) = LMM_BASE_PHYS + Offset;
	  ((struct dma_ctrl*)cgra.dma_ctrl)->ZDMA_CH_DST_DSCR_WORD2 = size*8*sizeof(U32);
...
void dma_read_U32(U64 Offset, U64 size){
	...
	int status;
	  *(U64*)&(((struct dma_ctrl*)cgra.dma_ctrl)->ZDMA_CH_SRC_DSCR_WORD0) = LMM_BASE_PHYS + Offset;
      ((struct dma_ctrl*)cgra.dma_ctrl)->ZDMA_CH_SRC_DSCR_WORD2 = size*8*sizeof(U32);
	  *(U64*)&(((struct dma_ctrl*)cgra.dma_ctrl)->ZDMA_CH_DST_DSCR_WORD0) = DDR_BASE_PHYS + Offset;
	  ((struct dma_ctrl*)cgra.dma_ctrl)->ZDMA_CH_DST_DSCR_WORD2 = size*8*sizeof(U32);
	  ((struct dma_ctrl*)cgra.dma_ctrl)->ZDMA_CH_CTRL2 = 1;
	        do {
	  status = ((struct dma_ctrl*)cgra.dma_ctrl)->ZDMA_CH_STATUS & 3;
...
void dma_write_U64(U64 Offset, U64 size){
	...
	int status;
	  *(U64*)&(((struct dma_ctrl*)cgra.dma_ctrl)->ZDMA_CH_SRC_DSCR_WORD0) = DDR_BASE_PHYS + Offset;
      ((struct dma_ctrl*)cgra.dma_ctrl)->ZDMA_CH_SRC_DSCR_WORD2 = size*4*sizeof(U64);
	  *(U64*)&(((struct dma_ctrl*)cgra.dma_ctrl)->ZDMA_CH_DST_DSCR_WORD0) = LMM_BASE_PHYS + Offset;
	  ((struct dma_ctrl*)cgra.dma_ctrl)->ZDMA_CH_DST_DSCR_WORD2 = size*4*sizeof(U64);
	  ((struct dma_ctrl*)cgra.dma_ctrl)->ZDMA_CH_CTRL2 = 1;
	        do {
	  status = ((struct dma_ctrl*)cgra.dma_ctrl)->ZDMA_CH_STATUS & 3;
      } while (status != 0 && status != 3);
